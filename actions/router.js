...
router.post("/", validateAction, (req, res) => {
  Actions.insert(req.body)
    ...
    .catch((err) => {console.log(`Failed to insert action: ${err}`);
...
router.put("/:id", getActionById, validateActionChange, (req, res) => {
  Actions.update(req.action.id, req.body)
    ...
    .catch((err) => {console.log(`Failed to update action: ${err}`);
...
function validateAction(req, res, next) {
  if (req.body.description === undefined || req.body.notes === undefined) {
    res
      .status(400)
      .json({ error: " a 'description' and 'notes' must be given" });
    return;
  }
  Projects.get(req.body.project_id)
    .then((project) => {
      if (project !== null) {
        ...
        res
          .status(400)
          .json({ error: "No project exists with id" });
      }
    })
    .catch((err) => {
