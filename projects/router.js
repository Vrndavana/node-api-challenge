...
router.delete("/:id", getProjectById, (req, res) => {
  Projects.remove(req.project.id)
    .then((num_deleted) => {
      if (num_deleted > 0) {res.status(200).json(req.project);} else {
...
function getProjectById(req, res, next) {
  Projects.get(req.params.id)
    .then((project) => {
      if (project !== null) {req.project = project;
        next();} else {
        res.status(404).json({ error: "No project with the specified id" });}})
    .catch((err) => {res.status(500).json({ error: "Could not retrieve projects data" });
    });
}
...
function validateProject(req, res, next) {
  if (req.body.name !== undefined && req.body.description !== undefined) {
    next();} else {
    res.status(400).json({ error: "The fields 'name' and 'description' must be given" });
  }
}
